...
const CALM_TIPS: readonly { icon: typeof Moon; text: string }[] = [
  ...
  },
  {
    icon: Wind,
    text: typo(
      "Волнение перед дверью — нормально. Дыхание 4-7-8: вдох на 4 счёта, задержка на 7, выдох на 8 — три круга, и пульс уляжется.",
    ),
  },
];
...
function ExamDayPage() {
  ...
  const introText = exam.examDate
    ...
    : typo(
        "Основная работа уже сделана. Сегодня только спокойная поддержка: короткое повторение, разгрузка головы и вода с собой.",
      );
